...
class HindsightExperienceReplayWrapper(object):
    ...
    def __init__(self, replay_buffer, n_sampled_goal, goal_selection_strategy, wrapped_env, relative=False, goal_range_low=None, goal_range_high=None, her_clip_goals=False):
        ...
        self.her_clip_goals = her_clip_goals
        self.goal_range_low = goal_range_low
        if not self.goal_range_low is None:
            self.goal_range_low = np.array(self.goal_range_low)
        self.goal_range_high = goal_range_high
        if not self.goal_range_high is None:
            self.goal_range_high = np.array(self.goal_range_high)
        self.relative = relative
        if self.relative:
            print("#################################")
            print("# Notice: using relative buffer #")
            print("#################################")
        else:
            print("Note: HER is not in relative mode.")
    ...
    def _store_episode(self):
        ...
        for transition_idx, transition in enumerate(self.episode_transitions):
            ...
            if self.relative:
                # We're going to have to split them apart and rejoin.
                obs, action, reward, next_obs, done, info = copy.deepcopy(transition) # could maybe copy less
                obs_dict, next_obs_dict = map(self.env.convert_obs_to_dict, (obs, next_obs))
                # assert obs_dict['desired_goal'] == next_obs_dict['desired_goal'], "Error, desired goals were not the same for same episode transition"
                # print("Transition in:", obs_dict, next_obs_dict)
                obs_dict['achieved_goal'] = obs_dict['desired_goal'] - obs_dict['achieved_goal']
                next_obs_dict['achieved_goal'] = next_obs_dict['desired_goal'] - next_obs_dict['achieved_goal']
                obs_dict['desired_goal'] = np.zeros(obs_dict['desired_goal'].shape)
                next_obs_dict['desired_goal'] = np.zeros(next_obs_dict['desired_goal'].shape)
                # print("Transition out:", obs_dict, next_obs_dict)
            ...
            for goal in sampled_goals:
                ...
                if self.her_clip_goals and not self.goal_range_low is None and not self.goal_range_high is None:
                    goal = np.clip(goal, self.goal_range_low, self.goal_range_high)
                    # Check for the XYZ Skill
                    if hasattr(self.env.env, "xyz_skill"):
                        if goal[0]**2 + goal[1]**2 > 0.09**2:
                            goal[2] = 0
                        else:
                            goal[2] = np.sqrt(0.09**2 - goal[0]**2 - goal[1]**2)
                ...
                if self.relative:
                    # Convert all experience to be relative to the location of the goal. IE goal = 0, achieved_goal = goal - achieved_goal
                    obs_dict['achieved_goal'] = goal - obs_dict['achieved_goal']
                    next_obs_dict['achieved_goal'] = goal - next_obs_dict['achieved_goal']
                    obs_dict['desired_goal'] = np.zeros(goal.shape)
                    next_obs_dict['desired_goal'] = np.zeros(goal.shape)
